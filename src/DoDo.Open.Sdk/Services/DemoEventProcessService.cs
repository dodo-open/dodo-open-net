...
namespace DoDo.Open.Sdk.Services
{
    ...
    public class DemoEventProcessService : IEventProcessService
    {
        ...
        public void Received(string message)
        {
            ...
            var type = Convert.ToInt32(jObject["type"]);

            if (type == EventSubjectDataTypeConst.Business)
            {
                var eventType = Convert.ToInt32(jObject["data"]?["eventType"]);
                if (eventType == EventTypeConst.ChannelMessage)
                {
                    var messageType = Convert.ToInt32(jObject["data"]?["eventBody"]?["messageType"]);
                    if (messageType == MessageTypeConst.Text)
                    {
                        var result = JsonConvert.DeserializeObject<EventSubjectOutput<EventSubjectDataBusiness<EventBodyChannelMessage<MessageText>>>>(message);
                        if (result != null)
                        {
                            var eventBody = result.Data.EventBody;
                            var messageBody = result.Data.EventBody.MessageBody;
                    ...
                    else if (messageType == MessageTypeConst.Photo)
                    {
                        var result = JsonConvert.DeserializeObject<EventSubjectOutput<EventSubjectDataBusiness<EventBodyChannelMessage<MessagePhoto>>>>(message);
                        if (result != null)
                        {
                            var eventBody = result.Data.EventBody;
                            var messageBody = result.Data.EventBody.MessageBody;
                    ...
                    else if (messageType == MessageTypeConst.Video)
                    {
                        var result = JsonConvert.DeserializeObject<EventSubjectOutput<EventSubjectDataBusiness<EventBodyChannelMessage<MessageVideo>>>>(message);
                        if (result != null)
                        {
                            var eventBody = result.Data.EventBody;
                            var messageBody = result.Data.EventBody.MessageBody;
            ...
            var timer = new Timer(x =>
            {
                foreach (var output in outputList)
                {
                    _openApiService.SendChannelMessage(new SendChannelMessageInput<MessageText>()
                    {
                        ...
                        MessageBody = new MessageText
                        {
                            Content = $"现在是北京时间：{DateTime.Now:yyyy-MM-dd HH:mm:ss}"
                        }
                    });
                }
